const imagePreloads = images.map(imagePath => {
...
function updateImage() {
 ...
 currentImage.src = imagePreloads[currentIndex].src
  currentImage.src = imagePath;

  const imageNumberElement = document.getElementById('image-number');
  
  if (currentIndex === 248) {
    imageNumberElement.textContent = 'Image 249 (final canvas)';
  } else {
    imageNumberElement.textContent = `Image ${currentIndex + 1}`;
  }
}
...
infoCloseBtn.addEventListener('click', hideImageInfo);

const infoBtn = document.getElementById('info-btn');
infoBtn.addEventListener('click', showImageInfo);

let imageDates = {}; // Store image dates from JSON
...
function updateImageInfo(imagePath) {
  ...
  image.src = imagePath;
  image.onload = () => {
    const imageName = imagePath.split('/').pop(); // Extract the image name from the path
    const imageDate = imageDates[imageName] || 'Unknown Date'; // Get image date from JSON
    document.getElementById('image-name').textContent = `Image Name: ${imageName}`;
    document.getElementById('image-date').textContent = `Date Created: ${imageDate}`;
  };
}
...
function showPrevImage() {
  currentIndex = (currentIndex - 1 + images.length) % images.length;
  updateImage();
  updateImageInfo(images[currentIndex]);
}
...
function updateImageInfo(imagePath) {
  ...
  image.onload = () => {
    const imageName = imagePath.split('/').pop(); // Extract the image name from the path
    getImageDate(image).then(imageDate => {
      document.getElementById('image-name').textContent = `Image Name: ${imageName}`;
      document.getElementById('image-date').textContent = `Date Created: ${imageDate}`;
      infoBox.style.display = 'block';
    });
...
async function getImageDate(image) {
  const exifMetadata = await getExifData(image);
  if (exifMetadata && exifMetadata.exif && exifMetadata.exif.DateTimeOriginal) {
    const dateString = exifMetadata.exif.DateTimeOriginal.description;
